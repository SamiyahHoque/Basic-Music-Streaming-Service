    ...
    */
    initEditToolbarHandlers() {        
        ...
        document.getElementById("add-button").onmousedown = (event) => {
            this.model.addAddSongTransaction("Untitled", "Unknown", "dQw4w9WgXcQ", this.model.getPlaylistSize());
    ...
    */
    ...
    initModalHandlers(){
            ...
            deleteSongConfirmButton.onclick = (event) => {
                let title = this.model.currentList.getSongAt(this.model.currentSong).title;
                let artist = this.model.currentList.getSongAt(this.model.currentSong).artist;
                let id = this.model.currentList.getSongAt(this.model.currentSong).youTubeId;

                this.model.addRemoveSongTransaction(title, artist, id, this.model.currentSong);
            ...
            editSongConfirmButton.onclick = (event) => {            
                let titleButton = document.getElementById('title-button').value;
                let artistButton = document.getElementById('artist-button').value;
                let youTubeIdButton = document.getElementById('youTubeId-button').value;

                let oldTitle = this.model.currentList.getSongAt(this.model.currentSong).title;
                let oldArtist = this.model.currentList.getSongAt(this.model.currentSong).artist;
                let oldId = this.model.currentList.getSongAt(this.model.currentSong).youTubeId;

                this.model.addEditSongTransaction(oldTitle, oldArtist, oldId, titleButton, artistButton, youTubeIdButton, this.model.currentSong);

                // CLEAR TEXT FIELDS
                document.getElementById('title-button').value = "";
                document.getElementById('artist-button').value = "";
                document.getElementById('youTubeId-button').value = "";
    ...
    registerItemHandlers() {
        ...
        for (let i = 0; i < this.model.getPlaylistSize(); i++) {
            ...
            deleteButton.onclick = (event) => {
                ...
                this.model.currentSong = i;

                let songName = this.model.currentList.getSongAt(i).title;
