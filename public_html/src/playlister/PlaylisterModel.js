...
export default class PlaylisterModel {
    ...
    */
    constructor() {
        ...
        // THIS IS THE SONG CURRENTLY BEING EDITED
    ...
    addNewSong(initName, initArtist, initId, initLocation) {
        if (this.hasCurrentList()) {
            let newSong = {"title": initName, "artist": initArtist, "youTubeId": initId};
            this.currentList.songs.splice(initLocation, 0, newSong);
            this.view.refreshPlaylist(this.currentList);
        }
        this.saveLists();
    }

    editSong(initName, initArtist, initId, initLocation) {
        if (this.hasCurrentList()) {
            let newSong = {"title": initName, "artist": initArtist, "youTubeId": initId};
            this.currentList.setSongAt(initLocation, newSong);
            this.view.refreshPlaylist(this.currentList);
        }
        this.saveLists();
    }

    removeSong(initLocation) {
        if (this.hasCurrentList()) {
            this.currentList.songs.splice(initLocation, 1);
            this.view.refreshPlaylist(this.currentList);
        }
        this.saveLists();
    }
    ...
    addAddSongTransaction(initName, initArtist, initId, initLocation) {
        let transaction = new AddSong_Transaction(this, initName, initArtist, initId, initLocation);
        this.tps.addTransaction(transaction);
        this.view.updateToolbarButtons(this);
    }

    addEditSongTransaction(initOldName, initOldArtist, initOldId, initNewName, initNewArtist, initNewId, initLocation) {
        let transaction = new EditSong_Transaction(this, initOldName, initOldArtist, initOldId, initNewName, initNewArtist, initNewId, initLocation);
        this.tps.addTransaction(transaction);
        this.view.updateToolbarButtons(this);
    }

    addRemoveSongTransaction(initName, initArtist, initId, initLocation) {
        let transaction = new RemoveSong_Transaction(this, initName, initArtist, initId, initLocation);
        this.tps.addTransaction(transaction);
        this.view.updateToolbarButtons(this);
    }
}